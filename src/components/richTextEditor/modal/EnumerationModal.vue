  ...
  mounted() {
    ...
    if (selection.length) {
      ...
      if (ttsData) {
        ...
        const resultList = [];
        const tmp = inputString.split('###').filter(s => s);
        tmp.forEach((e) => {
          resultList.push(e.split('//'));
        });
        let res = resultList.reduce((prev, cur) => {
          if (prev.length === 0) {
            return prev;
          }
          if (cur.length === 0) {
        ...
        res = res.join('//');
        const newTTSData = {
          normalized_texts: [res],
          origin_text: ttsResult[0] || '',
          from: range.index,
          to: range.index + range.length,
        };
        this.currentTTSData = newTTSData;
        this.ttsNormalizedData = res;
      }
    }
    setTimeout(() => {
      this.renderTTS();
